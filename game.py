...
def phase(match_state: dict, left_team: Team, right_team: Team,
    ...
    while True:
        match_state["phase"] += 1
        current_carrier, current_running_team = creating_competition(left_team, right_team,
                                                                     game_table)
        result = turn(match_state, current_carrier, current_running_team,
                      left_team, right_team, game_table, silent)
        if result == "Touchdown":
            break


def game(left_team: Team, right_team: Team, league_table: DataFrame, declare: dict) -> DataFrame:
    game_table, left_team_players, right_team_players, state_dict = prepare_match(left_team, right_team)
